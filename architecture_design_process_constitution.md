...
**The same interpretive authority, guardrails, and citation discipline cover Scenarios (glossary §12A, added per DEC-1430).** A **Scenario** — a concrete intended-usage narrative that grounds and disambiguates the requirements that *enact* it — carries the same interpretive, non-generative, non-gating authority as an Aim, and the two guardrails above bind it identically: a Scenario is cited *alongside* the requirement it grounds, never *instead* of one (T1) — "this matches our Scenario of X" is not a substitute for a requirement that is absent — and a genuine Scenario↔requirement clash routes to **Requirement reconsideration** (T3), never a silent reinterpretation past the requirement's plain meaning. The SPLIT exit's *"scope or intent ambiguity"* triage may consult the requirement's governing **Aim(s) or Scenario(s)** — a concrete worked example of the intended use is often exactly what disambiguates an ambiguous requirement — citing the AIM- or SCN- id it leaned on. A Scenario differs from an Aim in one respect: it is **non-adjudicative**. Where an Aim adjudicates a conflict among otherwise-valid candidates, a Scenario may only *surface* one — a usage it shows to be unsatisfiable while two requirements both hold — leaving the adjudication itself to the Aim layer (§4, glossary §12A).
...
- every Aim (glossary §12) is enacted by at least one requirement,
  ...
  rationale;
- every Scenario (glossary §12A) is enacted by at least one
  requirement, or has been explicitly retired or deferred with
  recorded rationale.
```

The last criterion is the **dual of the driving question** (added per DEC-1420): the driving question checks bottom-up whether every requirement is fulfilled; this criterion checks top-down whether every Aim **and every Scenario (glossary §12A)** is *enacted* — whether at least one requirement pursues each stated intent or intended usage. An Aim with no enacting requirement signals that the design may not yet match the project's real intent (a coverage gap), or that the Aim is stale. Such an Aim is **dispositioned like an assumption** (§15.1) — **enacted** (a requirement is written or identified that enacts it), **retired** (§30A / glossary §29 — the intent no longer holds), or **deferred** with recorded rationale (intended but not yet pursued) — never silently left orphaned. The structural check (does at least one *enacts* edge point to the Aim?) is auditable by the tool; whether the enacting requirements *faithfully* realize the Aim's intent remains a judgment at the design-done review (an Aim carries no pass/fail verification — glossary §12). A **Scenario** with no enacting requirement is dispositioned identically (§15.1) — enacted, retired, or deferred, never silently orphaned — and carries the same presence-not-truth limit: the structural check is whether at least one *enacts* edge points to it, while whether the enacting requirements faithfully realize its intended usage is a design-done judgment (a Scenario carries no pass/fail verification — glossary §12A). A Scenario may be temporarily un-enacted while design is in progress; the criterion binds only at design-done.

### 3.3 Hard Requirements

Hard requirements are pass/fail constraints. A candidate architecture that violates a hard requirement is invalid unless the requirement is changed by its owner. This validity test is **Aim- and Scenario-blind** (§8): neither an **Aim** (glossary §12) nor a **Scenario** (glossary §12A) ever softens, waives, or overrides a hard requirement — a hard requirement changes only when its owner changes it (§13).
...
**The Aim layer is blind to Stage 1 (added per DEC-1420).** The first stage — rejecting candidates that violate hard requirements (above; §3.3) — is **Aim-blind**. Alignment with an **Aim** (glossary §12) plays no part in whether a candidate is valid: a candidate that violates a hard requirement is rejected regardless of how well it aligns with any Aim. Aim-alignment enters **only at the second stage**, ranking among candidates that already pass *every* hard requirement (§8.1, §9.1). An Aim can therefore **tilt a tradeoff among valid candidates; it can never rescue an invalid one.** The same holds for a **Scenario** (glossary §12A): Stage 1 is Scenario-blind too — a candidate that violates a hard requirement is rejected regardless of how well it aligns with any Scenario, and Scenario-alignment, like Aim-alignment, enters only at the second stage and can tilt a tradeoff among valid candidates, never rescue an invalid one.
...
**An Aim is a citable interpretive input, but never carries a decision alone (added per DEC-1420).** An **Aim** (glossary §12) is an authoritative artifact and so may be *cited* in a justification — to explain *how* a requirement was interpreted, or *which* among already-valid candidates was preferred as better-**aligned** with the intent. But exactly parallel to the commonness rule above, **an Aim alone never carries the decision**: Aim-alignment may sharpen or tilt a justification already grounded in a requirement, constraint, or tradeoff — it never stands in for one. A selection whose sole justification is "this better serves our Aim of X" fails this rule just as a typicality-only justification does. When an Aim is leaned on, its AIM- id is cited (LB · REQ-0110). A **Scenario** (glossary §12A) is a citable interpretive input in exactly the same way: it may sharpen or tilt a justification already grounded in a requirement, constraint, or tradeoff, but a Scenario alone never carries the decision and never stands in for one — "this matches our Scenario of X" as the sole justification fails this rule just as a typicality-only justification does. When a Scenario is leaned on, its SCN- id is cited (LB · REQ-0110).
...
**Aim-alignment as a criterion (added per DEC-1420).** Alignment with an **Aim** (glossary §12) may appear as a §9.1 criterion, but it scores candidates as better- or worse-**aligned** — never pass/fail. An Aim never gates: hard-requirement rejection is §8's Stage 1, which is Aim-blind (§3.3, §8). An Aim is **not** itself a standalone grounding line under this sub-rule; an Aim-alignment criterion is grounded through one of (a)-(d) above — typically (a), tracing to the requirement(s) that *enact* the Aim or to the Aim's governing REQ/DEC — exactly as any other criterion is grounded. The Aim's AIM- id is cited on the criterion (LB · REQ-0110). **Scenario-alignment** (glossary §12A) may likewise appear as a §9.1 criterion, scoring candidates as better- or worse-**aligned** — never pass/fail (hard-requirement rejection is §8's Stage 1, which is Scenario-blind — §3.3, §8). Like Aim-alignment, a Scenario is **not** a standalone grounding line: a Scenario-alignment criterion is grounded through one of (a)-(d) above — typically (a), tracing to the requirement(s) that *enact* the Scenario. The Scenario's SCN- id is cited on the criterion (LB · REQ-0110).
...
- Detecting un-enacted scenarios (no requirement enacts a stated Scenario)
