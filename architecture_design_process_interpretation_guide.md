...
### §9.1 — criterion-set completeness: the criterion that was never written down

**Governing decision:** DEC-2000 (grounded in PO-55; empirical case DEC-1990; promotion gate PO-39). **See also:** §9.1 (criterion-grounding — the provenance of the columns present; cell-correspondence — the truth of the entries present; top-priority-ranking [PO-36 / DEC-0750] — the nearest neighbour; the anti-gaming heuristics — the motive of a present criterion), §9.3 (where the check gates), §2A Stage 2 (the Compare-stage completeness risk it closes, firing at Stage 3's moment), §3.2 (no silent assumptions; hybrid synthesis, the highest-risk locus), §10.4 (trivial derivation, where the pass does not fire), PO-31 (noticing-dependence), PO-39 (the fireable-form gate this passed).

**The seam.** Every other discipline §9.1 attaches to a matrix asks a question about something that is *on the page*: is this present criterion grounded, is this present cell true, was this correction applied symmetrically, is the present top-priority column represented. The criterion-set-completeness sub-rule asks about something that is *not* on the page — a criterion that would flip the recommendation but was never written down. That gap is invisible to every present-thing check: the matrix can be flawless by all of them and still recommend the wrong option, because the deciding axis is simply absent. It reads as the discipline working — the cells are grounded, the recommendation follows the cells — which is exactly why it is dangerous.

**Why it is not already covered.** The obvious candidate is the top-priority-ranking sub-rule (PO-36 / DEC-0750), and the difference is worth stating precisely, because it is the crux of this sub-rule's right to exist (PO-39(c)). PO-36 reaches a *missing top priority*, and it reaches it *when the analyst notices* one is missing — the same noticing a context-limited agent performs unreliably (PO-31). This sub-rule is broader on both axes: it reaches *any* decision-flipping criterion, not only a top priority; and it *forces* the question at a fixed moment rather than waiting to be noticed. It also adds a second limb PO-36 has no analogue for — redundancy: a recommended option that merely duplicates a mechanism the system already has is not warranted by its own margin. That limb is not decoration; it is the half that would have caught the empirical case.

**The worked example (DEC-1990).** The project proposed, twice and with confidence, a connect-time guard against spec-drift writes. Its §9.1 matrix passed every existing check both times. It was wrong: two decision-flipping criteria were absent — the guard's marginal value *over F7's existing write-time gate* (near zero: F7 already fail-closes the only gate-visible hazard, so limb (b)'s redundancy was the decisive omission), and the operator cost it imposed against a hazard never demonstrated. Adding those two axes flipped the recommendation to the status quo. Nothing in the check surfaced the gap; it surfaced only when the external authority asked *"is there more we should have captured?"* This sub-rule turns that question from a lucky prompt into a recorded obligation.

**What a recorded pass does not buy.** Like the cell-correspondence and Scenario-consultation sub-rules and §10.4's contradiction pass, this one is *presence, not truth*: it certifies the two-limb pass was performed and recorded, not that the stated flipping-criterion (or the "none found" attestation) is correct. And it is weaker than cell-correspondence in one honest respect the promotion faced squarely: cell-correspondence removed noticing entirely (every cell carries a mark, unconditionally), but *completeness of a criterion set is not mechanically enumerable* — you cannot list every criterion that could flip a decision — so the best available form only forces the adversarial question at a fixed moment. A decisive criterion can still be missed in good faith. That residual is the sub-rule's stated limit, not a defect awaiting a stronger form; none exists. What the rule removes is the dependence on *happening to think of the question* — the question is now always asked, at ratification, and its absence is FAIL-MUST-REVISE.

### §3.2 / §12A / §9.1 — "deferred" names two different things: an enactment disposition (still Active, still walked) and a parked, no-longer-Active usage (not walked)

**Governing decision:** DEC-1970 (grounded in the terminology seam surfaced while separating the tool's enactment-exemption axis from the §29 lifecycle axis). **See also:** §3.2 *Termination* (the enactment-coverage criterion and its three dispositions), glossary §12A (Scenario *Completeness* + *Lifecycle*), §12 (Aim), §29 (Status = the lifecycle axis), §30A (retirement as a lifecycle transition), the §9.1 *Scenario consultation* entry above (whose "Deferred Scenario" coverage hole this disambiguates), §9.3 (where the walk gates).

**The seam.** The word *deferred* appears in the constitution in two places that a reader can easily collapse into one, and they are not the same thing. §3.2 *Termination* and glossary §12A use it as one of the three **enactment dispositions** of an Aim or Scenario — *enacted, retired, or deferred with recorded rationale*. The §9.1 *Scenario-consultation* entry above, listing its coverage holes, uses it of a **"Deferred Scenario"** that "falls outside an Active-only walk." Read carelessly, those two sentences seem to contradict: one says a deferred intent is a live, still-intended thing carried to a later cycle; the other says a Deferred Scenario is parked and off the walk. The contradiction is only apparent, and dissolving it turns on seeing that "deferred" is doing two different jobs on two different axes.

**The resolved reading — two axes, one discriminator.** An Aim or Scenario carries two independent axes. Its **lifecycle status** (§29) — Draft / Proposed / Active / Rejected / Superseded / Deprecated / Under Review — says what governance state the artifact is in. Its **enactment disposition** (§3.2 *Termination* / §12A) — enacted / retired / deferred — says whether a requirement yet pursues its intent. These are orthogonal, and *deferred* lives on the second axis, not the first.

- **Sense 1 — enactment-deferred (§3.2 / §12A).** This is a statement about *enactment*, not about lifecycle. The intent **still holds**: it is intended but not yet pursued by any requirement, recorded with rationale, to be picked up in a later cycle. §12A is explicit that this is ordinary work-in-progress — a Scenario "may be temporarily un-enacted *while design is in progress*." An enactment-deferred Aim/Scenario is therefore **still §29-Active.** Because the §9.1 Scenario-consultation walk enumerates *every §29-Active Scenario* (that entry's own words: "status per glossary §29"), an enactment-deferred Scenario **remains in the walk.** The only thing its deferral exempts is the §3.2 *Termination* **enactment-coverage** check at design-done — never the §9.1 consultation, and never anything else that keys on §29-Active.

- **Sense 2 — a parked, no-longer-Active usage (the §9.1 "Deferred Scenario").** The coverage-hole sentence in the entry above is describing a *different* artifact: one the project has **parked** — "the parked set," "work nobody intends to do." Such a Scenario is out of the walk **because its §29 lifecycle status is not Active**, not because the word "deferred" carries any walk-excluding force of its own. A parked usage is one whose intent the owner is no longer pursuing in this design; disposing of it honestly means moving it *off* §29-Active (retiring it under §30A, or otherwise recording it as a non-Active state) — at which point the §29-Active walk correctly stops visiting it.

**The discriminator a §9.1 reader uses.** Walk-membership is decided by the **§29 lifecycle status alone**: *is this Scenario Active?* The enactment disposition is orthogonal to that question. An enactment-deferred **but §29-Active** Scenario **is walked**. A **§29-non-Active** Scenario — parked, retired, superseded, deprecated, rejected, or not-yet-Active — **is not walked**, whatever anyone has called it colloquially. So a reader who meets the phrase "deferred Scenario" asks not "was the word *deferred* used?" but "is it §29-Active?" — and the two senses separate cleanly.

**Retired vs deferred, on the same map.** §12A ties *retired* to "§30A / §29 — the intended usage no longer holds." Retirement is thus a **lifecycle transition off Active** (a §29/§30A move), and it takes the Scenario out of the §9.1 walk for the ordinary reason (it is no longer §29-Active). Enactment-**deferral** does the opposite: it leaves the artifact §29-Active and in the walk, exempting only the enactment-coverage criterion. This is exactly why the §9.1 entry's coverage hole notes that when a decision irreversibly forecloses a usage narrated by a **parked** Scenario, "the only honest disposition left is to retire it" — retiring is a §29-Active exit, and it is that exit, not the word "deferred," that removes the Scenario from future walks.

**Honest boundary — this entry mints nothing.** Like every entry in this Guide, this one is interpretive and non-authoritative. It introduces no rule, no new §29 status value (in particular it does **not** make "Deferred" a lifecycle status — §29's list is unchanged), no artifact type, and no operational mechanism. It records which of two pre-existing constitutional senses of "deferred" a §29-keyed reader is in, and confirms the discriminator the constitution and glossary already imply: §29 lifecycle status governs walk-membership; the enactment disposition governs only enactment-coverage. Where §29 lacks a crisp terminal value for a *retired* intent, that is a §29 question this entry deliberately does not settle (see the governing DEC's open item).
