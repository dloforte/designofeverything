...
An **artifact** is any governed item recorded under this process — a requirement, aim, scenario, decision, contract, catalog option, capability statement, assumption, clarification, open question, traceability link, change request, fulfillment check, invalidation history entry, or any other type the process recognizes. Every artifact is uniquely addressable, owned, versioned, and traceable.
...
A **Scenario** (§12A) — a concrete intended-usage narrative — may be the recorded narrative **basis** of a requirement's *Demonstration*: the end-to-end script the demonstration walks through. This does not make the Scenario a verification artifact. The Demonstration verifies the **requirement**, and the pass/fail stays on the requirement↔candidate relationship (the candidate satisfies or violates the requirement); the Scenario itself is non-gating and is never passed or failed (§12A).
...
- Scenarios (§12A) — the concrete intended-usage narratives in the
  same non-gating intent layer — sit OFF this gradient rather than
  on a rung of it. A Scenario is concrete and specific, but it is
  non-normative: it narrates intended use and makes no "shall"
  statement, so the Measurable and Time-bound criteria that grade
  requirement statements do not apply to it. The Aim and the
  Scenario bracket the non-normative intent space — the Aim as
  abstract direction, the Scenario as a concrete worked example of
  use — while the requirements that enact them carry the normative
  SMART obligations.
...
An **Aim (§12)** or a **Scenario (§12A)** is a further interpretive parent that, like those context artifacts, does **not** confer authority. A requirement may record the Aim(s) and/or Scenario(s) it *enacts* (§1A): the Aim or Scenario is the requirement's *interpretive* parent — the "why" it exists, or the worked example of intended use it realizes — and this is explicitly **not** a third authority origin. The two authority origins above (Root, Derived) are unchanged. An Aim or Scenario never originates or confers requirement authority, never generates a requirement, and never gates a candidate. The *enacts* reference and the authority-origin reference are two separate edges on the requirement — a **two-parent firewall** that keeps intent interpretive and keeps requirements the sole authoritative drivers of the design.
...
The concrete sibling of the Aim — the Scenario (use-case) — is defined in §12A.

## 12A. Scenario

A **scenario** is a first-class artifact recording a concrete intended-usage narrative — a worked example of the intended use — that grounds and disambiguates the requirements that enact it. Its display identifier is `SCN-NNNN`. A Scenario is the **concrete sibling of the Aim (§12)** in the same non-gating intent layer: like an Aim, it is an artifact of intent that requirements *enact*; the two differ only in abstraction. An Aim is abstract direction — the "why"; a Scenario is concrete intended usage — a worked example of *what using the result looks like*, end to end. (For a curriculum-design project a Scenario might narrate a learner's path through a module; for a hardware project, an operator's end-to-end use of the device in its setting; for a software project, a user's journey through a feature.)

A Scenario carries **interpretive and parental authority**: it governs how an ambiguous requirement is read, and — exactly as an Aim is — it is the interpretive parent of the requirements that enact it (§4, §1A). Unlike an Aim, a Scenario is **non-adjudicative**: it can *surface* a conflict — a required behaviour it shows to be unsatisfiable across two requirements — but it does not adjudicate one; the substrate for adjudicating a conflict among otherwise-valid candidates remains the Aim layer (§12). This authority is deliberately **non-generative and non-gating**.

A Scenario is **never pass/fail**. Unlike a requirement, a Scenario is not something a candidate can *satisfy* or *violate*; a candidate or a requirement is only better or worse **aligned** with a Scenario (a satisficing relationship). The verbs *satisfies* and *violates* stay reserved for the requirement↔candidate relationship (§13, §1A). A Scenario therefore never gates a candidate and never brings a requirement into being — it grounds interpretation only.

A Scenario is the **interpretive parent, never the authority parent.** A requirement records the Scenario(s) it *enacts* (§1A) — the **same** reserved role by which a requirement records the Aim(s) it enacts, not a separate role — as an upward "worked-example-of-use" reference, distinct from, and additional to, its §4 authority origin (root or derived) and its §7 derivation parent. Because a Scenario is admitted only as an interpretive parent and never as a third authority origin, it can never confer authority on a requirement or generate one. §4's two authority origins are unchanged (see §4 for the two-parent firewall, which covers Aims and Scenarios alike).

**Scenarios and Demonstration verification.** A Scenario is the natural narrative for the **Demonstration** verification method (§2) — the "user-journey / end-to-end" method — yet the intent layer is non-gating, so the two are kept as **distinct roles**. A Demonstration passes or fails a **requirement**: it is *how* that requirement is verified, and the pass/fail lives on the requirement↔candidate relationship (a candidate satisfies or violates the requirement). A Scenario may be the recorded narrative **basis** of such a Demonstration — the end-to-end script the demonstration walks through — but the gate is always on the requirement, never on the Scenario. The Scenario is walked, not passed or failed (see §2).

**Type-level no-verification.** A Scenario is not independently verified pass/fail. As a requirement may record a verification waiver (§2), and as the Aim type carries one (§12), the Scenario type carries a type-level waiver: a Scenario is verified *indirectly*, through the requirements that enact it and the Demonstrations those requirements may script. Its correctness lives in whether the enacting requirements, taken together, faithfully realize the intended usage it narrates.
...
Per constitution **§13.3** (added by DEC-1620), a change request additionally carries, from its issuer at creation, a coarse **downstream-impact estimate** (heuristic and fallible — a suggestion for how possibly-affected work proceeds, never a gate: it neither validates nor invalidates a candidate, §8/§8.1). While it remains **pending** — issued but not yet ruled on — the request (a) **marks** its governing artifact and their transitive downstream dependents as having a pending change against them (pull-surfaced, retracted if the request is rejected or modified — the pending-window complement to the landed-window review of §13.2), and (b) accrues an **application-priority** that rises with its **age**, with an outcome-forcing terminal that guarantees the affected owner *rules* it (accept / reject / modify) rather than letting it sit unaddressed — silence is never consent, and the owner retains full reject/veto authority (§26).
